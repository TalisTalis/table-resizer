...
class TableResizer {
    ...
    loadColumnWidths(instance) {
        ...
        if (savedWidths && savedWidths.length > 0) {
            ...
            const regularHeaders = table.querySelectorAll('th:not(.column-control-header)');
            const regularColumnsCount = regularHeaders.length;

            if (regularColumnsCount > 0) {
                if (regularColumnsCount > options.minColumnsForDefaultWidth) {
                    // Если столбцов больше 4 - устанавливаем дефолтную ширину
                    ...
                    // Если столбцов 4 или меньше - используем ширину экрана / количество столбцов
                    const widthScreen = window.innerWidth - 80;
                    const widthColumn = Math.floor(widthScreen / regularColumnsCount);
                    ...
                    regularHeaders.forEach((header, index) => {
                        const allHeaders = table.querySelectorAll('th');
                        let actualIndex = Array.from(allHeaders).indexOf(header);
                        this.setColumnWidth(instance, actualIndex, widthColumn + 'px');
                    });
                }
    ...
    distributeColumns(tableId) {
        ...
        const { table } = instance;
        const headers = table.querySelectorAll('th:not(.column-control-header)');
        const regularColumnsCount = headers.length;
        ...
        if (regularColumnsCount > 0) {
            ...
            headers.forEach((header, index) => {
                const allHeaders = table.querySelectorAll('th');
                let actualIndex = Array.from(allHeaders).indexOf(header);
                this.setColumnWidth(instance, actualIndex, finalWidth + 'px');
            });
    ...
    reset(tableId) {
        ...
        if (instance) {
            ...
            const regularHeaders = instance.table.querySelectorAll('th:not(.column-control-header)');
            const regularColumnsCount = regularHeaders.length;

            if (regularColumnsCount > 0) {
                if (regularColumnsCount > instance.options.minColumnsForDefaultWidth) {
                    // Если столбцов больше 4 - устанавливаем дефолтную ширину
                    regularHeaders.forEach((header, index) => {
                        const allHeaders = instance.table.querySelectorAll('th');
                        let actualIndex = Array.from(allHeaders).indexOf(header);
